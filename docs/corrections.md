...
| [reference.md#the-hula-drive](reference.md#the-hula-drive) | **A hula pair with no path of contact came back with a row of zeroes** | A contact ratio of zero, an efficiency of zero, and that zero fed to the power flow as the pair's fixed-carrier efficiency — from which the drive reported an efficiency, a set of shaft torques, a backlash and three speeds, none of which mean anything and none of which said so. It is refused now, as it is in every other stage kind. The find is worth the entry for how it surfaced: a test asserting that a hula's two modules want to be equal had been comparing one drive that transmits against four that do not, and it only failed once the refusal existed |

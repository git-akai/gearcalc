...
| `planetary.rs` | The set's **vocabulary** — sun, carrier, ring; an arrangement — Pennestrì's closed form, the independent check the shape's flow is held to on every arrangement, and `carrier_driven_efficiency`, the 3K family's closed form the hula laws hold the flow to. No solve: the set's closure is the shape's | tooth form, geometry |
...
| File | Answers |
|---|---|
| `mod.rs` | **What every stage shares**: the load cases — each a list of `Load`s on the train's open ports, given or derived, solved in `solve_train` as one motion and one flow across every stage and handed down as a `CaseLoad` per stage; `TrainCase` is what it comes to body by body, `PathReport` what the train comes to along each path the ends or a case ask for (`paths_of`), and `Train::relieve_case` is the relief that keeps a case to the train's mobility — `StageLoads` (the lone-stage asked form), `MemberRating`, `Bending`, `MeshReport`, `GearResult` and its `GearCase` per load, the engagement rule, and the train that strings stages together — a stage **is** a `Shape` and a stage's result **is** a `ShapeResult`, with the shared machinery written as `impl Shape` here — and **relief**: `Freedom`, `Reading`, `FreedomGroup` and the walk over them, which a stage feeds and never writes. Six questions are the whole of what a stage owes (`shape.rs`, once the `Constrained` trait): the fifth is its wiring and the sixth its ports |
| `wiring.rs` | **Where a stage's slots and meshes sit** — topology alone, no geometry, feeding `kinematics.rs`; `add_to` is handed the slot-to-body lookup, which is the stage's own list of bodies and the identity for a stage asked alone. The frame is *derived* from the two members' common frame rather than stored, and a mesh's sign is its `MeshKind`'s |
| `conditions.rs` | **What the train holds, and its bodies**: a `BodyConstraint` (held or free) on a body — a number across the train, ground 0, which every stage, case and hold names; a body listed on two stages is what a coupling used to say, and `Train::chained` is the constructor that numbers a chain's — and the `StageBoundary` each stage is handed, its own reporting convention, derived and never stored. `Train::motion` (the shaft line, driven at the train's first end where it has two), `open_ports`, `bodies` (every port body with its ends), `ends`, `port` (a body by stage and slot), and the **edits** the panel asks through `edit_train` — `join`, `split`, `hold`, `release`, `move_end` (the select's one rule), `push_stage`, `remove_stage`, `edit_stage`, `fresh_case`, `set_duty` — each a rule about what else has to change, and every remove closing the numbers up (`prune`). Nothing is driven but by a load |
...
| `arrangements.rs` | **The arrangements the shape reaches with no code of their own**, and the **presets** the menu offers: `epicyclic` is the one builder for the family — the central members and the carrier in shaft order, which is the order the conventions read — and `line` for parallel axes; a Wolfrom, a stepped planet, a planocentric, meshed planets, a Ravigneaux, a layshaft, a worm feeding a spur pair and the hula stage (a stepped Wolfrom at one planet, at the proportions its family runs at — no preset, and the harness's `hula` commands build it from the list) are lists over them. the pair's and the set's vocabularies (`PairStage`, `PlanetaryStage`) are the same two lists with their words written on. `StagePreset` is a variant per menu entry with its family (`StageFamily`, which `Shape::family` reads off any shape), its name's key and its starting shape; `defaults()` crosses `StagePreset::ALL` | anything about solving |
| `edits.rs` | **What a card does to a stage** — `StageEdit`: a step, a sun or a ring on a planet gear, an axis, a pair added or removed, a member moved to a body on its axis — each a rule about what else changes, on the shape, a body it adds numbered after the train's (`next`); a body it takes off its last stage leaves the train through `Train::edit_stage` (in `conditions.rs`), which closes the numbers up. A refusal (`EditRefused`) names its invariant and changes nothing. No kind flips: a swap is a remove and an add, sized by the core |
...
| `pair.rs` | The pair **vocabulary** (`PairStage`, a worm by one bit), what `defaults` builds a spur or a worm from, and the undercut bound a search or an absorber is held to. No solve, no chooser — the tests' door onto the shape is `cfg(test)` |
| `planetary.rs` | The set **preset**: three members, the bodies they sit on, the boundary a convention gives it. No solve |
...
| **A stage arrangement** | none of the core, if the shape already holds it: a preset is a list in `arrangements.rs`, a `StagePreset` variant with its family and label, and one string ×5 — `defaults()` crosses the list and the menu renders it, pushed onto a train by `Train::push_stage`; its members are named by `Shape::member_names`. What the shape cannot yet lay out — a member on two axes, a planet–planet assembly rule — is a change to `shape.rs`, and **nothing in `MemberRating`, `MeshReport`, `Bending` or `GearResult` should move** | `cargo nextest run` — `every_arrangement_of_a_set_solves` and the relief laws sweep every preset and every arrangement · `gear-cli kinematics` and `tools/train_kinematics.py` · the corpus |
| **A stage edit** | `train/edits.rs` — the variant, its rule and its refusal · `edit_train`'s `Stage` arm in `gear-wasm` and a step in `tools/wasm_probe.mjs` · the button in `web/src/TrainPanel.svelte` · 5 × `strings_*.toml` | `cargo nextest run` — `every_add_on_every_preset_solves`, `every_add_undoes` and `a_refused_edit_changes_nothing` sweep the presets · `tools/check_wasm.sh --write` |

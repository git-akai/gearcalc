...
| `conditions.rs` | **What the train holds over a stage's conventions**: a `Constraint` on a `ShaftRef` (held, driven, free), a `Coupling` between two stages' shafts, and the `StageBoundary` each stage is handed — derived, never stored. `Train::motion` — the shaft line off the graph — and `Train::open_ports` — every shaft a load can enter by, a shared shaft listed once — live here; a hold or a drive the train writes on a stage replaces the stage's convention of that kind; a load names nothing — the chain's ends are the convention's, so a load written at `End` stays on one shaft as loads are added beside it |
...
| `crates/gear-wasm` | The boundary. 20 entry points, JSON in and JSON out, all pure — `tools/wasm_boundary.json` lists them, and `check_wasm.sh` fails on one it does not |

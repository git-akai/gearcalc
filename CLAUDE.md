...
| File | Answers |
|---|---|
| `mod.rs` | **The train, and what it comes to**: one graph (`Train::shape`), its holds and its load cases — each a list of `Load`s on the train's open ports, given or derived — solved in `solve_train` part by part (`solve_parts`) under one motion and one flow across the whole graph, each part handed its share as a `CaseLoad`. `TrainCase` is what a case comes to body by body; `PathReport` what the train comes to along each path a case asks for (`paths_of`) — its ratio, its efficiency with breaking away asked of the whole flow, its play, the power through its teeth, and what one more tooth on each gear does, by the graph's index; `Train::relieve_case` is the relief that keeps a case to the train's mobility. `TrainResult` answers per piece — every gear, mesh, distance and axis by the graph's index, what is a part's own (`PartReport`), and `part`/`by_part`, a part's view laid back out for the harness — through `MemberRating`, `Bending`, `MeshReport`, `GearResult` and its `GearCase` per load; the engagement rule; `Train::alone`, a preset asked alone as a train of one (its bodies numbered by its slots, cased at its conventional ends, solved as one part; `arranged` asks it held elsewhere, `solve_alone` reads its figures off the path, as `Alone`); and **relief** — `Freedom`, `Reading`, `FreedomGroup` and the walk over them, which the shape feeds and never writes. Six questions are the whole of what a shape owes (`shape.rs`, once the `Constrained` trait): the fifth is its wiring and the sixth its ports |
| `graph.rs` | **What the graph falls apart into**: `Shape::parts`, the pieces that close, search and rate apart — members joined by meshes, meshes by the distances they share — each a `Part` carrying its shape in its own numbering and where every piece of it is in the graph: what a stage was, and the solve's unit, never the designer's. `Part::whole`, a shape as one part; the graph's own surgery (`append`, `merge_axes`, `remove_part`); and **`graph_of`, a file of stages as one shape** — a body two stages shared listed once, the fixed axes it turned about one line, a join that cannot be coaxial an offset coupling — which is how a file written as stages converts (`gear-cli convert`); its laws hold the converted graph to what a chain builds, to the stages' motion body for body, and to their solves part by part |
...
| `planetary.rs` | **The set's words as a boundary** (`planetary_boundary`): sun, carrier, ring — the words the harness and the shape's laws ask a lone set in — as the conditions its slots take. The set itself is `arrangements::planetary`. No solve |

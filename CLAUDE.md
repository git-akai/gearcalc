...
| `graph.rs` | **What a stage was**: `Shape::parts`, the pieces of the graph that close, search and rate apart — members joined by meshes, meshes by distances — each a `Part` carrying its shape in its own numbering and where every piece of it is in the graph; `Part::whole`, a shape as one part; and the graph's own surgery (`append`, `merge_axes`, `remove_part`). And **`graph_of`, a list of stages as one shape** — a body two stages share listed once, the fixed axes it turns about one line, a join that cannot be coaxial an offset coupling — which is how a file written as stages converts; its laws hold the converted graph to what a chain builds now, to the stages' motion body for body, and to their solves part by part |

...
| `wiring.rs` | **Where a stage's slots and meshes sit** — topology alone, no geometry, feeding `kinematics.rs`; `Slots` maps a stage's slots to the train's bodies. The frame is *derived* from the two members' common frame rather than stored, and a mesh's sign is its `MeshKind`'s |

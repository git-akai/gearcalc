...
It exists because the project is about 20,800 lines of production code carrying
14,400 lines of comment, alongside 5,800 lines of standalone document — **prose
...
train-as-graph branch began, and 20,800 / 14,400 / 5,800 at its head — so
that branch added about 3,700 lines of production code against 800 of
comment. That is a fifth of the crate's own density, and the reason is
where it went: `shape.rs` gained 2,900 lines of solve for what five stage
types used to do separately, while `planetary.rs`, `hula.rs`, `pair.rs` and
...
| `edits.rs` | **What a card does to a stage** — `StageEdit`: a step, a sun or a ring on a planet gear, an axis, a mesh added or removed, a member moved to a body on its axis, an orbiting body coupled to a shaft on its carrier's axis and uncoupled — each a rule about what else changes, on the shape, a body it adds numbered after the train's (`next`); a body it takes off its last stage leaves the train through `Train::edit_stage` (in `conditions.rs`), which closes the numbers up. **A gear moved off a body does not take the body with it** — a body is a port the train may hold, share or load, and dropping it because its gear moved is how engaging a layshaft's other ratio lost the output; a body nothing names at all is given up by `Train::drop_bare`, a level up, where what else names it can be seen. A refusal (`EditRefused`) names its invariant and changes nothing. No kind flips: a swap is a remove and an add, sized by the core |
| `graph.rs` | **The train as one shape** (`Train::graph`): every stage's pieces in one list, a body two stages share listed once and the fixed axes it turns about merged — a shaft is straight — and a join that cannot be coaxial made an offset coupling; `Pieces` says where each stage's went. And `Shape::parts`, the pieces that close, search and rate apart — members joined by meshes, meshes by distances — which for a train of stages are its stages. Derived until the train holds the graph: its laws hold it to the train's motion, exactly, and to its stages, part by part |
...
| **An edit the panel asks for** | a *stage's*: `train/edits.rs` — the `StageEdit` variant, its rule and its refusal · `edit_train`'s `Stage` arm in `gear-wasm` and a step in `tools/wasm_probe.mjs` · the button in `web/src/TrainPanel.svelte` · 5 × `strings_*.toml`. A *train's*: the method on `Train` in `train/conditions.rs` — what else has to change is the rule — · a `TrainEdit` variant beside it · the same probe step and panel control | `cargo nextest run` — `every_add_on_every_preset_solves`, `every_add_undoes` and `a_refused_edit_changes_nothing` sweep the presets; the train's edits have their laws in `train/mod.rs` and `train/edits.rs` · `tools/check_wasm.sh --write` |

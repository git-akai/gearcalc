...
| `graph.rs` | **What the graph falls apart into**: `Shape::parts`, the pieces that close, search and rate apart — members joined by meshes, meshes by the distances they share — each a `Part` carrying its shape in its own numbering and where every piece of it is in the graph: what a stage was, and the solve's unit, never the designer's. the graph's own surgery (`append`, `merge_axes`, `remove_part`); and **`graph_of`, a file of stages as one shape** — a body two stages shared listed once, the fixed axes it turned about one line, a join that cannot be coaxial an offset coupling — which is how a file written as stages converts (`gear-cli convert`); its laws hold the converted graph to what a chain builds, to the stages' motion body for body, and to their solves part by part |

...
| File | Answers |
|---|---|
| `mod.rs` | **What every kind shares**: the load cases and the walk that carries each along its route to the far end, `StageLoads`, `MemberRating`, `Bending`, `MeshReport`, `GearResult` and its `GearCase` per load, the engagement rule, the train that strings stages together — and **relief**: `Freedom`, `Reading`, `FreedomGroup` and the walk over them, which each kind feeds through `Constrained` and never writes. `Constrained` is the whole of what a kind owes: six questions: the fifth is its wiring and the sixth its ports |
...
| `hula.rs` | The hula stage: the arrangement built, cut and rated. Kinematically a compound planet with one planet and both meshes internal, which its `wiring` says in one place |
...
| **A stage kind** | its own file — the assembly and the result it keeps — plus `Constrained`'s six answers, of which `wiring()` is the whole of its kinematics and `ports()` is what a constraint may address. **Nothing in `MemberRating`, `MeshReport`, `Bending` or `GearResult` should move**; if it does, the division `docs/rationale.md#each-stage-kind-keeps-its-own-result-type` argues was wrong | `cargo nextest run` — `the_graph_gives_every_kind_the_kinematics_it_gives_itself` sweeps every kind and arrangement · `tools/train_kinematics.py` · the corpus |

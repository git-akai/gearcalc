...
| **An edit the panel asks for** | a *stage's*: `train/edits.rs` — the `StageEdit` variant, its rule and its refusal, and its index read through the part in `Shape::edit_part` · `edit_train`'s `Stage` arm in `gear-wasm` and a step in `tools/wasm_probe.mjs` · the button in `web/src/TrainPanel.svelte` · 5 × `strings_*.toml`. A *train's*: the method on `Train` in `train/conditions.rs` — what else has to change is the rule — · a `TrainEdit` variant beside it · the same probe step and panel control | `cargo nextest run` — `every_add_on_every_preset_solves`, `every_add_undoes` and `a_refused_edit_changes_nothing` sweep the presets, and `an_edit_on_a_card_is_the_edit_on_its_stage` asks each add of a train's second card; the train's edits have their laws in `train/mod.rs` and `train/edits.rs` · `tools/check_wasm.sh --write` |

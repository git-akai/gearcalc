...
| `edits.rs` | **What a card does to a stage** — `StageEdit`: a step, a sun or a ring on a planet gear, an axis, a pair added or removed, a member moved to a shaft of its axis — each a rule about what else changes, on the shape; `Renumbered` is what a remove moved where, which `Train::edit_stage` (in `conditions.rs`) repoints the cases, couplings and constraints by. A refusal (`EditRefused`) names its invariant and changes nothing. No kind flips: a swap is a remove and an add, sized by the core |

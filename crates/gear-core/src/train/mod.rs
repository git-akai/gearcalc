...
impl Stage {
    /// A pair — spur, helical, crossed, or a worm and its wheel where the
    /// pair says so — as a shape.
    #[must_use]
    pub fn pair(p: PairStage) -> Self {
...
/// other, **where its bodies and meshes sit**, and **which bodies a train may
...
pub(crate) trait Constrained {
    ...
    /// **Where the stage's bodies and meshes sit** — topology alone, with no
...
impl Stage {
    ...
    /// **Where this stage's bodies and meshes sit** ([`Wiring`]) — the topology
...
pub struct StageLoads {
    ...
    /// **Which of the stage's bodies are held and which driven**, assembled by
...
pub fn solve_any_with(
    ...
    // constraints — every held body held — and each load asked of it is a
    ...
    // The stage alone: its own numbering is the train's.
    let port = |shaft: Body| stage.as_shape().map_or(shaft, |s| s.body_at(shaft));
    let mut train = Train {
        load_cases: Vec::new(),
        ...
        constraints: boundary
            .held()
            .into_iter()
            .map(|s| BodyConstraint::held(port(s)))
            .collect(),
    };
...
mod tests {
    ...
    fn every_member_of_a_reacting_stage_reports_its_share() {
        ...
        // between it and the output carries it.
        {
            ...
            s.insert(0, Stage::pair(PairStage::worm()));
            restage(&mut train, s);
        }
        train.push_stage(Stage::planetary(PlanetaryStage::default()));
        train.push_stage(Stage::pair(PairStage {
            shaft_angle: 90.0,
            ..PairStage::default()
        }));
    ...
    fn a_back_driving_load_reaches_the_wheel_undiminished() {
        ...
        let driven = |input_torque: f64, back: f64| {
            ...
            restage(&mut train, vec![Stage::pair(PairStage::worm())]);
            let r = solve_train(&train, &lib).expect("a train that solves");
    ...
    fn an_internal_mesh_is_asked_what_an_internal_mesh_is_asked() {
        ...
        for (stage, internal) in [
            (Stage::pair(PairStage::default()), vec![false]),
            (
                Stage::planetary(PlanetaryStage::default()),
                vec![false, true],
            ),
            ...
            (Stage::pair(PairStage::worm()), vec![false]),
        ] {
    ...
    fn every_wiring() -> Vec<(String, Stage, StageBoundary)> {
        ...
        for (z1, z2) in [(17_u32, 43_u32), (9, 37), (13, 13)] {
            ...
            s.gears[0].teeth = z1;
            s.gears[1].teeth = z2;
            out.push(conventional(format!("spur {z1}/{z2}"), Stage::pair(s)));
        }
        out.push(conventional("worm".into(), Stage::pair(PairStage::worm())));
    ...
    fn a_reversing_stage_does_not_poison_the_stages_around_it() {
        ...
        let mut t = train_of(vec![reversing(), Stage::pair(PairStage::default())]);
        ...
        let mut t = train_of(vec![Stage::pair(PairStage::default()), reversing()]);
    ...
    fn a_trains_ratio_carries_the_direction_its_output_turns() {
        ...
        assert!(one(Stage::pair(PairStage::default())) < 0.0);
        ...
        assert!(one(Stage::pair(PairStage::worm())) < 0.0);
    ...
    fn a_ports_select_is_one_rule_split_then_held_joined_or_its_own() {
        ...
        let mut t = train_of(vec![set(), Stage::pair(PairStage::default())]);
        let before = t.clone();
    ...
    fn a_hold_replaces_the_conventions_hold_and_a_shared_body_says_where_a_stage_is_entered() {
        ...
        let mut t = Train::chained(vec![Stage::pair(PairStage::default()), set()], |_| {
            Vec::new()
        });
    ...
    fn the_cases_survive_the_last_stage_and_take_the_next_one_conventionally() {
        let lib = library();
        let mut t = train_of(vec![Stage::pair(PairStage::default())]);
        t.load_cases.truncate(1);
    ...
    fn the_surviving_reading_is_the_one_the_solve_reads() {
        ...
        let relieved = Stage::pair(both).relieved(None);
    ...
    fn a_ratio_given_on_straight_teeth_says_it_asked_nothing() {
        ...
        let mut t = train_of(vec![Stage::pair(sp.clone())]);
        let r = solve_train(&t, &lib).expect("solves");
        ...
        restage(&mut t, vec![Stage::pair(helical)]);
        let r = solve_train(&t, &lib).expect("solves");
    ...
    fn a_distance_the_shifts_cannot_reach_is_said_out_loud() {
        ...
        let at = |a: f64| {
            ...
            sp.gears[0].teeth = 9;
            sp.gears[1].teeth = 37;
            let t = train_of(vec![Stage::pair(sp)]);
    ...
    fn the_declared_limit_is_the_freedom_the_stage_actually_has() {
        ...
        let groups = Stage::pair(PairStage::default()).freedoms();
        let relation = groups
            .iter()
        ...
        for distance in [None, Some(30.3_f64), Some(30.5)] {
            for clearance in [0.0_f64, 0.02, 0.20] {
                for optimiser in [false, true] {
                    ...
                    let t = train_of(vec![Stage::pair(sp), Stage::pair(PairStage::worm())]);
                    let Ok(r) = solve_train(&t, &lib) else {
                        continue;
                    };
    ...
    fn a_back_driven_hula_distributes_torque_by_its_own_solve() {
        ...
        let ratios = |mu: f64| {
            ...
            restage(
                &mut t,
                vec![Stage::pair(PairStage::worm()), Stage::Shape(Box::new(h))],
            );
    ...
    fn a_back_driven_set_distributes_torque_by_its_own_solve() {
        ...
        let ratios = |mu: f64| {
            ...
            restage(
                &mut t,
                vec![Stage::pair(PairStage::worm()), Stage::planetary(set)],
            );
            let r = solve_train(&t, &lib).expect("a train that solves");
    ...
    fn a_parallel_stage_is_rated_at_the_centre_distance_it_runs_at() {
        ...
        for clearance in [0.0_f64, 0.02, 0.1, 0.3] {
            let r = solve_pair_stage(&stage(clearance), &StageLoads::just(2.0), &lib).unwrap();
    ...
    fn the_automatic_face_width_is_the_larger_of_the_enabled_checks() {
        ...
        let width = |sources: FaceSources| {
            ...
            solve_pair_stage(&s, &StageLoads::just(2.0), &lib)
                .unwrap()
                .members[0]
                .face_width
        };
    ...
    fn a_width_with_no_rating_to_size_it_stands_where_it_was() {
        ...
        let spur_r = solve_pair_stage(&spur, &StageLoads::just(2.0), &lib).unwrap();
    ...
    fn the_sharing_model_reaches_every_member_that_bends() {
        ...
        let bending_of = |sharing: LoadSharing| {
            ...
            let s = solve_pair_stage(&spur, &StageLoads::just(2.0), &lib).unwrap();
    ...
    fn a_stage_that_did_not_ask_keeps_the_shifts_it_had() {
        ...
        let lib = library();
        let loss = |on: bool| {
            1.0 - solve_pair_stage(&stage(on), &StageLoads::just(2.0), &lib)
                .unwrap()
                .meshes[0]
                .efficiency
                .forward
    ...
    fn the_clearance_is_taken_by_whatever_is_free_to_absorb_it() {
        ...
        let pinned = solve_pair_stage(&at(false), &StageLoads::just(2.0), &lib).unwrap();
        assert!(
        ...
        let chosen = solve_pair_stage(&at(true), &StageLoads::just(2.0), &lib).unwrap();
    ...
    fn a_train_that_fails_names_the_stage_that_failed() {
        let mut train = two_stage();
        train.push_stage(Stage::pair(PairStage::default()));
        assert!(
    ...
    fn a_material_override_changes_the_answer() {
        ...
        let auto_width = |sources: FaceSources, o: Overrides| {
            ...
            solve_pair_stage(&s, &StageLoads::just(2.0), &lib).unwrap()
        };
    ...
    fn a_back_driving_load_is_carried_only_where_something_reacts_it() {
        ...
        t.push_stage(Stage::pair(PairStage {
    ...
    fn a_train_with_no_load_case_is_a_shaft_line() {
        ...
        for off in [false, true] {
            let mut train = two_stage();
            train.push_stage(Stage::pair(PairStage::worm()));
            train.push_stage(Stage::planetary(PlanetaryStage::default()));
            train.push_stage(hula());
